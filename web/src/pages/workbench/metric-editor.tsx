...
function ShapeEditor({
  ...
  return (
    <div className="flex flex-col gap-2">
      <span className="text-xs font-medium text-[var(--kf-text-secondary)]">这个指标怎么算？</span>

      <label
        className={cx(
          'flex flex-wrap items-center gap-2.5 rounded-md border bg-[var(--kf-bg-container)] px-2.5 py-2',
          shape === 'column' && !freeform
            ? 'border-[var(--kf-primary-border)] ring-2 ring-[var(--kf-primary-bg)]'
            : 'border-[var(--kf-border-secondary)]',
        )}
      >
        <input
          type="radio"
        ...
        />
        <span className="whitespace-nowrap text-sm text-[var(--kf-text)]">对一列做统计</span>
        {shape === 'column' && !freeform && (
          <span className="ml-auto flex items-center gap-2">
            ...
            <span className="text-xs text-[var(--kf-text-tertiary)]">的</span>
            <Select
              className="h-8 w-auto"
      ...
      <label
        className={cx(
          'flex flex-col gap-2 rounded-md border bg-[var(--kf-bg-container)] px-2.5 py-2',
          shape === 'metric' || freeform ? 'border-[var(--kf-primary-border)] ring-2 ring-[var(--kf-primary-bg)]' : 'border-[var(--kf-border-secondary)]',
        )}
      >
        <span className="flex items-center gap-2.5">
          ...
          />
          <span className="text-sm text-[var(--kf-text)]">由其它指标计算</span>
          {freeform && (
            <span className="ml-auto text-xs text-[var(--kf-warning-text)]">
              这条口径选择器表达不了，按原文编辑
            </span>
          )}
        </span>
  ...
  if (parsed === null) {
    ...
    return (
      <Field label="只统计满足条件的行" hint="这条条件选择器表达不了，保持原样；清空可改用选择器">
        <div className="flex items-center gap-2">
          <span className="flex-1 truncate rounded-md border border-[var(--kf-border-secondary)] bg-[rgb(var(--kf-fill-alter-rgb))] px-3 py-2 font-mono text-xs text-[var(--kf-text-secondary)]">
            {filterSql}
          </span>
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-baseline justify-between">
        <span className="text-xs font-medium text-[var(--kf-text-secondary)]">只统计满足条件的行</span>
        <span className="text-xs text-[var(--kf-text-tertiary)]">留空 = 全部行</span>
      </div>
        ...
        return (
          <div key={index} className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-[var(--kf-text-tertiary)] hover:bg-[rgb(var(--kf-fill-tertiary-rgb))] hover:text-[var(--kf-text-secondary)]"
            >
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-[var(--kf-text-tertiary)]">取值从已发布的维度值里选，不写 SQL</span>
      </div>
    </div>
  );
}
...
export function MetricEditor({
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-3 rounded-md border border-[var(--kf-border-secondary)] bg-[rgb(var(--kf-fill-alter-rgb))] px-3 py-3">
        <div className="flex items-center justify-between gap-2">
          <span className="flex items-center gap-2">
            <span className="text-sm font-medium text-[var(--kf-text)]">口径定义</span>
            <Badge tone={form.metricDefineType === 'METRIC' ? 'slate' : 'sky'} variant="outline">
              {form.metricDefineType === 'METRIC' ? '复合指标' : '原子指标'}
            </Badge>
          </span>
          <span className="text-xs text-[var(--kf-warning-text)]">改动会直接改变问数结果</span>
        </div>
        <ShapeEditor
          values={form}
          columns={columns}
      ...
      {creating && (
        ...
        >
          ...
          />
          {bizNameError && <div className="mt-1 text-xs text-[var(--kf-error-text)]">{bizNameError}</div>}
        </Field>
      )}
      ...
      <button
        ...
        className="flex w-fit items-center gap-1.5 text-xs text-[var(--kf-text-secondary)] hover:text-[var(--kf-text)]"
      >
        <ChevronRight className={cx('h-3 w-3 transition-transform', showMore && 'rotate-90')} />
        更多（展示格式、小数位、敏感度、业务分类）
      </button>
      {showMore && (
        <div className="flex flex-col gap-3 rounded-md border border-[var(--kf-border-secondary)] px-3 py-3">
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-3">
            ...
            {form.dataFormatType === 'percent' && (
              <Field label="数值换算" hint="存的是 0.3 就勾上">
                <label className="flex h-9 items-center gap-2 text-sm text-[var(--kf-text-secondary)]">
                  <input
                    type="checkbox"
