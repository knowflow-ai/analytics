...
# 生成 / 校正 SQL 的读超时。带 CTE 的条件占比查询在自建模型上要 25 到 35 秒，30 秒常年
# 卡在边上，而超时这一档不重试——一次超时就是整条问不出来。
export KNOWFLOW_ANALYTICS_MODEL_GATEWAY_QUERY_TIMEOUT_SECONDS="${ANALYTICS_QUERY_TIMEOUT:-60}"
