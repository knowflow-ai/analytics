    ...
    return (
      <div className="mx-auto max-w-2xl px-6 py-12">
        <div className="flex flex-col items-center text-center">
          <span className="grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br from-[var(--kf-primary)] to-[var(--kf-primary-hover)] text-[var(--kf-text-light-solid)] shadow-md">
            <Sparkles className="h-6 w-6" />
          </span>
          <h2 className="mt-4 text-base font-semibold text-[var(--kf-text)]">AI 自动建模</h2>
          <p className="mt-2 text-xs leading-relaxed text-[var(--kf-text-secondary)]">
            一次运行完成四件事：为每张表和字段起业务名称、判定字段角色（标识 / 维度 / 时间 / 度量）并生成指标，
            为指标、维度和常见维度值生成别名，补充可审核的语义上下文，再由完整目录确定性编译安全的查询作用域。
            目录结果只是草稿；查询作用域是只读预览。
          </p>
          {job.data?.status === 'failed' && (
            <div className="mt-4 w-full rounded-md border border-[var(--kf-error-border)] bg-[var(--kf-error-bg)] px-3 py-2 text-left text-xs text-[var(--kf-error-text)]">
              上次运行失败：{job.data.error ?? '未知错误'}
            </div>
          )}
          {(jobStale || stale) && (
            <div className="mt-4 text-xs text-[var(--kf-warning-text)]">模型在生成建议后又被修改过，需要重新运行。</div>
          )}
          {applied && (
            <div className="mt-4 text-xs text-[var(--kf-success-text)]">上次建议已采用。可以再次运行以获取新的建议。</div>
          )}
          <Button
            variant="primary"
            className="mt-6"
          ...
          <div className="mt-3 text-xs text-[var(--kf-text-tertiary)]">
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-10">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-sm font-semibold text-[var(--kf-text)]">AI 建模进行中 · {stageLabel}</div>
          <div className="mt-0.5 text-xs text-[var(--kf-text-tertiary)]">{hint}</div>
        </div>
        <Button size="sm" loading={cancelling} onClick={onCancel}>
          取消
        </Button>
      </div>
      <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-[rgb(var(--kf-fill-tertiary-rgb))]">
        <div
          className={`h-full rounded-full bg-[var(--kf-primary)] transition-all ${stage === 'enriching' ? 'animate-pulse' : ''}`}
          style={{ width: `${percent}%` }}
        />
      </div>
      <ul className="mt-5 divide-y divide-[var(--kf-border-secondary)] rounded-lg border border-[var(--kf-border-secondary)]">
        {tables.map((table) => (
          <li key={table.model_id} className="flex items-center justify-between px-3 py-2 text-xs">
            <span className="text-[var(--kf-text)]">{table.name}</span>
            <Badge
              tone={
...
function ProposalReview({
      ...
      {items.map((item) => {
        ...
        return (
          ...
          >
            <input
              type="checkbox"
              className="mt-0.5 accent-[var(--kf-primary)]"
              checked={!off}
              onChange={() => toggle(item.id)}
            />
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 text-[var(--kf-text)]">
              ...
              <div className="mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5 text-[var(--kf-text-secondary)]">
                {lines.map((line) => (
                  <span key={line}>{line}</span>
                ))}
              </div>
              {item.reason && <div className="mt-0.5 text-xs text-[var(--kf-text-tertiary)]">{item.reason}</div>}
            </div>
          </li>
        );
      })}
  ...
  return (
    <div className={`flex flex-col ${ANALYTICS_TASK_PANEL_CLASS}`}>
      {/* 总结条：先说清这次要你做什么 */}
      <div className="flex items-center gap-3.5 border-b border-[var(--kf-border-secondary)] bg-gradient-to-r from-[var(--kf-primary-bg)] to-transparent px-5 py-3">
        <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-gradient-to-br from-[var(--kf-primary)] to-[var(--kf-primary-hover)] text-[var(--kf-text-light-solid)]">
          <Sparkles className="h-4 w-4" />
        </span>
        <div className="min-w-0 flex-1">
          <div className="text-sm font-semibold text-[var(--kf-text)]">
            AI 读完了 {spec.models.length} 个实体，给出 {proposal.suggestions.length} 条建议
          </div>
          <div className="mt-0.5 text-xs leading-relaxed text-[var(--kf-text-secondary)]">
            名字和角色多数照数据库注释与主键推出来，可以直接过；
            {aliasDrafts.length > 0 && (
              <b className="font-semibold text-[var(--kf-warning-text)]">
      ...
      <div className="grid h-[calc(100vh-420px)] min-h-[400px] grid-cols-[220px_minmax(0,1fr)]">
        {/* 左：分区清单 */}
        <nav className="overflow-auto border-r border-[var(--kf-border-secondary)] bg-[rgb(var(--kf-fill-alter-rgb)/0.4)] p-2.5">
          <div className="px-2 pb-2 text-xs font-semibold uppercase tracking-wide text-[var(--kf-text-tertiary)]">
            这次要过目的
          </div>
          {sections.map((item) => {
            ...
            return (
              <button
                ...
                className={`mb-0.5 flex w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-left transition-colors ${
                  on ? 'bg-[var(--kf-primary-bg)]' : 'hover:bg-[rgb(var(--kf-fill-tertiary-rgb))]'
                }`}
              >
                <span
                  className={`shrink-0 ${on ? 'text-[var(--kf-primary)]' : item.muted ? 'text-[var(--kf-text-quaternary)]' : 'text-[var(--kf-text-tertiary)]'}`}
                >
                  {item.icon}
                </span>
                <span className="min-w-0 flex-1">
                  <span
                    className={`block text-xs font-medium ${
                      on ? 'text-[var(--kf-primary-active)]' : item.muted ? 'text-[var(--kf-text-tertiary)]' : 'text-[var(--kf-text)]'
                    }`}
                  >
                    {item.label}
                  </span>
                  <span className="mt-0.5 block truncate text-xs text-[var(--kf-text-tertiary)]">
                    {item.note}
                  </span>
                </span>
          ...
          <div className="mt-3 border-t border-[var(--kf-border-secondary)] px-2 pt-3 text-xs leading-relaxed text-[var(--kf-text-tertiary)]">
            采用后写进<b className="font-semibold text-[var(--kf-text-secondary)]">草稿版本</b>，不会动线上。
            取消勾选的条目保留你原来的值。
          </div>
        </nav>
        ...
        <div className="flex min-w-0 flex-col">
          <div className="flex items-center gap-2.5 border-b border-[var(--kf-border-secondary)] px-5 pb-2.5 pt-3">
            <h3 className="text-sm font-semibold text-[var(--kf-text)]">{meta?.title}</h3>
            <span className="text-xs text-[var(--kf-text-tertiary)]">{meta?.desc}</span>
          ...
          <div className="min-h-0 flex-1 overflow-auto px-5 py-3">
            {current === 'naming' && (
              <div className="flex flex-col gap-4">
                {grouped.model.length > 0 && (
                  <section>
                    <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-[var(--kf-text-tertiary)]">
            ...
            {current === 'alias' && (
              <div>
                <div className="mb-3 flex items-start gap-2 rounded-lg border border-[var(--kf-warning-border)] bg-[rgb(var(--kf-warning-bg-rgb)/0.6)] px-3 py-2">
                  <Info className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[var(--kf-warning-text)]" />
                  <span className="text-xs leading-relaxed text-[var(--kf-warning-text)]">
                ...
                {emptyValueDrafts.length > 0 && (
                  <div className="mb-2 text-right">
                    <button
                      type="button"
                      className="text-xs text-[var(--kf-primary)] hover:text-[var(--kf-primary)]"
                    ...
                    return (
                      ...
                      >
                        <div className="min-w-0">
                          <div className="truncate font-medium text-[var(--kf-text)]">
                            {draft.resource_name}
                          </div>
            ...
            {current === 'context' && (
              <div>
                <p className="mb-2 text-xs leading-relaxed text-[var(--kf-text-tertiary)]">
                  每条上下文保留目标、类型与来源；确认后随版本冻结，问数时只使用与所选作用域相关的条目。
                  <b className="font-medium text-[var(--kf-text-secondary)]">来源不会被合并</b>
                  ...
                  {proposalContext.map((entry) => (
                    <li key={entry.id} className="px-3 py-2 text-xs">
                      <div className="flex flex-wrap items-center gap-1.5">
                        ...
                        <span className="font-mono text-xs text-[var(--kf-text-tertiary)]">{entry.target_id}</span>
                      </div>
                      <div className="mt-1 whitespace-pre-wrap leading-relaxed text-[var(--kf-text-secondary)]">
                        {entry.text}
                      </div>
                      {entry.source_ref && (
                        <div className="mt-1 break-all font-mono text-xs text-[var(--kf-text-tertiary)]">
                          {entry.source_ref}
                        </div>
                      )}
                    </li>
            ...
            {current === 'compiled' && (
              <div className="flex flex-col gap-4">
                <p className="text-xs leading-relaxed text-[var(--kf-text-tertiary)]">
                  这些是系统按完整目录确定性编译出来的执行范围，只在排查问题时用。
                  <b className="font-medium text-[var(--kf-text-secondary)]">日常建模不用看，也改不了</b>
                  ——要改就去改实体、关系、指标和维度。
                </p>
                <section>
                  <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-[var(--kf-text-tertiary)]">
                  ...
                  <ul className="rounded-lg border border-[var(--kf-border-secondary)] px-3 py-2 text-xs text-[var(--kf-text)]">
                    ...
                    {proposal.artifact.default_count_metrics.length === 0 && (
                      <li className="text-[var(--kf-text-tertiary)]">无（实体缺少已确认主标识）</li>
                    )}
                  </ul>
                </section>
                <section>
                  ...
                  <div className="mb-2 rounded-md border border-[var(--kf-primary-bg)] bg-[rgb(var(--kf-primary-bg-rgb)/0.6)] px-2.5 py-2 text-xs text-[var(--kf-primary-active)]">
                    <div>编译器 {proposal.artifact.query_scope_compiler_version}</div>
                    <div className="mt-0.5 break-all font-mono text-xs text-[var(--kf-primary-active)]">
                      {proposal.artifact.query_scope_compilation_hash ?? '旧版 artifact 未记录编译哈希'}
                    </div>
                  </div>
                  <ul className="divide-y divide-[var(--kf-border-secondary)] rounded-lg border border-[var(--kf-border-secondary)] px-3 text-xs text-[var(--kf-text)]">
                    {proposalScopes.map((scope) => (
                      <li key={scope.datasetId} className="py-2">
                        <div className="font-medium">{scope.datasetName}</div>
                        <div className="mt-1 grid gap-1 text-xs text-[var(--kf-text-secondary)]">
                          <div><b className="font-medium text-[var(--kf-text-tertiary)]">事实根</b> {scope.rootName}</div>
                          <div><b className="font-medium text-[var(--kf-text-tertiary)]">成员实体</b> {scope.modelNames.join('、') || '无'}</div>
                          <div><b className="font-medium text-[var(--kf-text-tertiary)]">指标</b> {scope.metricNames.join('、') || '无'}</div>
                          <div><b className="font-medium text-[var(--kf-text-tertiary)]">维度</b> {scope.dimensionNames.join('、') || '无'}</div>
                          <div><b className="font-medium text-[var(--kf-text-tertiary)]">默认计数</b> {scope.defaultCountName ?? '无（COUNT(*) 拒绝）'}</div>
                          <div><b className="font-medium text-[var(--kf-text-tertiary)]">关系路径</b> {scope.pathLabels.join('；') || '仅事实根'}</div>
                        </div>
                        <details className="mt-1.5 rounded bg-[rgb(var(--kf-fill-alter-rgb))] px-2 py-1 text-xs">
                          <summary className="cursor-pointer text-[var(--kf-text-secondary)]">规范名称 {scope.canonicalNames.length}</summary>
                          <ul className="mt-1 space-y-0.5 font-mono text-[var(--kf-text-secondary)]">
                            {scope.canonicalNames.map(([id, name]) => <li key={id}>{id} → {name}</li>)}
                            {scope.canonicalNames.length === 0 && <li>无</li>}
                          </ul>
                        </details>
                        <details className="mt-1 rounded bg-[rgb(var(--kf-fill-alter-rgb))] px-2 py-1 text-xs">
                          <summary className="cursor-pointer text-[var(--kf-text-secondary)]">编译排除项 {scope.exclusions.length}</summary>
                          <ul className="mt-1 space-y-0.5 text-[var(--kf-text-secondary)]">
                    ...
                    {proposalScopes.length === 0 && (
                      <li className="py-2 text-[var(--kf-text-tertiary)]">
      ...
      <div className="flex items-center gap-3.5 border-t border-[var(--kf-border-secondary)] bg-[rgb(var(--kf-fill-alter-rgb)/0.6)] px-5 py-2.5">
        <div className="flex items-baseline gap-1.5">
          <span className="text-base font-semibold text-[var(--kf-text)]">{accepted}</span>
          <span className="text-xs text-[var(--kf-text-tertiary)]">/ {proposal.suggestions.length} 条建议将被采用</span>
        </div>
