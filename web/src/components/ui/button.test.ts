...
describe('Button', () => {
  it('标签永远不换行', () => {
     ...
     * 按钮挤窄，「新建」裂成两行并从固定高度（Ant Design 默认 32px，h-8）里溢出，按钮整个变形。
     *
     * 挤不下时该让位的是文字，不是按钮。
     */
  ...
  it('高度固定，所以换行会溢出而不是撑高', () => {
    // 这条解释了上一条为什么必须成立：固定的 h-8 拦不住换行，只会让文字跑出边框。
    expect(render({}, 'x')).toContain('h-8');
  });

  it('图标与文字之间有间距', () => {
    expect(render({}, 'x')).toContain('gap-2');
  });
