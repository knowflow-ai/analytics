...
如果“销售额”同时命中多个同名业务定义，LLM 无法用名字表达选择，页面会展示一张面向业务人员的确认卡。

但确认卡是最后手段，不是常态。系统先用确定性规则消化歧义：精确命中所占的文本范围上剔除模糊召回；没有指标锚点时按证据归属确定分析范围——用户点名的是门店的属性，就不该被追问“你要分析销售单还是销售明细”。弱召回只作为提示进入模型 Prompt，不会升级成用户必须回答的问题。
