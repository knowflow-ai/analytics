...
> **开源的是语义建模层与受治理查询引擎**，包含建模工作台、AI 一键建模、发布与版本回滚、发布前验证和词汇缺口回流，以及底下那套 S2SQL 编译、治理关卡、只读 Guard 和查询 API。
>
> 面向业务用户的**问数助手、报表和多用户权限**不在开源版内，它们属于商业版 KnowFlow。两者共用同一份语义模型与同一个查询引擎，详见[开源版与商业版](#开源版与商业版)。
...
默认镜像是 [`knowflowai/analytics:v0.0.4`](https://hub.docker.com/r/knowflowai/analytics/tags?name=v0.0.4)，支持 `linux/amd64` 和 `linux/arm64`。
...
| 问数引擎 | 流式阶段推送、结果解读、文本 S2SQL 下钻续跑、默认时间窗可见可撤（引擎能力，开源版的入口是「问数验证」，对话式问数页在商业版） |
...
上表是开源版自带的能力。面向业务用户的问数助手、报表和多用户权限见[开源版与商业版](#开源版与商业版)。

---

## 开源版与商业版

开源版给的是**建模能力和查询引擎**。它自带一套完整的建模工作台，四步走完就能发布一份可用的语义模型，并在「问数验证」里用自然语言试问、逐题核对结果。

它不提供面向业务用户的问数产品面。日常提问的对话页、把回答钉成报表、按人分配数据权限，这三件事在商业版 KnowFlow 里。

| | 开源版 | 商业版 |
|---|---|---|
| 建模工作台（数据源、语义建模、问数验证、问数反馈） | 有 | 有 |
| AI 一键建模、别名与值字典建议 | 有 | 有 |
| 发布、版本回滚、发布前质量报告 | 有 | 有 |
| S2SQL 编译、治理关卡、只读 Guard、查询 API | 有 | 有 |
| PostgreSQL / MySQL / 上传表格，多数据源 | 有 | 有 |
| 词汇缺口收件箱、Golden Suite、诊断导出 | 有 | 有 |
| **问数助手**：对话式提问、多轮追问、下钻、结果解读、会话分享 | 无 | 有 |
| **报表**：把回答钉成卡片、按项目归档、刷新重跑、导出 Excel | 无 | 有 |
| **项目授权与数据范围**：多用户 RBAC、行级与列级权限 | 无 | 有 |
| 与知识库、Agent、企业账号体系打通 | 无 | 有 |
| 登录 | 单用户加可选共享口令 | 企业账号、LDAP / OIDC、企业微信 |

开源版能问数，入口在建模工作台的「问数验证」里，用途是发布前逐题核对模型建得对不对。商业版把同一个引擎接到面向业务用户的产品面上。

### 商业版长什么样

业务同事直接用中文提问，回答卡上写清系统怎么理解的，下面可以继续下钻。

![商业版问数助手：理解 chip、图表、数据表、结果解读与下钻](docs/screenshots/commercial-ask-assistant.png)

同比环比按带符号的增长率展示。紫色那个 chip 是系统自己补的理解，它把「销售」认成销售金额，并且把这件事说了出来。

![商业版问数助手：期间比与自动理解提示](docs/screenshots/commercial-period-ratio.png)

满意的回答钉成卡片，按项目归档成报表，随时刷新取最新数字，可整页导出 Excel。

![商业版报表：钉住的问数结果，可刷新与导出](docs/screenshots/commercial-reports.png)

商业版还提供项目级授权与行列级数据范围，同一个项目里不同的人看到的行和列由授权决定。

想试用或了解商业版，可以走[官网](https://www.knowflowchat.cn)，或者加微信 `skycode007` 备注「问数」。
...
| 变量 | 默认值 | 作用 |
|---|---|---|
| `KNOWFLOW_ANALYTICS_IMAGE` | `knowflowai/analytics:v0.0.4` | 要运行的 Analytics 镜像 |
...
- 独立版是单用户 + 可选共享口令，不提供多用户 RBAC 或行列权限；需要按人分配数据范围请看商业版。
