...
class _FailureStore:
    ...
    def save_failure(self, record, **kwargs) -> None:
        self.saved.append((record, kwargs))
...
class _IndependentWeakPhraseEmbeddingGateway:
    """Keep one business phrase embedding-only while another is lexical."""

    def encode(self, texts: tuple[str, ...]) -> EmbeddingBatch:
        def vector(text: str) -> tuple[float, ...]:
            values = [0.0] * 128
            if "火星" in text or "商品动销" in text:
                values[0] = 1.0
            elif "销售额" in text or "净收入" in text:
                values[1] = 1.0
            else:
                values[2 + (sum(ord(char) for char in text) % 126)] = 1.0
            return tuple(values)

        return EmbeddingBatch(
            model_id="independent-weak-phrases",
            dimension=128,
...
class _PhraseChoosingIntentAdjudicator:
    def __init__(
        self,
        labels_by_phrase: dict[str, str],
        *,
        abstain_phrases: frozenset[str] = frozenset(),
    ) -> None:
        self.labels_by_phrase = labels_by_phrase
        self.abstain_phrases = abstain_phrases
        self.calls: list[dict[str, object]] = []

    def adjudicate(self, **kwargs) -> IntentAdjudicationResult:
        self.calls.append(kwargs)
        detected_text = kwargs["detected_text"]
        target = next(
            label
            for phrase, label in self.labels_by_phrase.items()
            if phrase in detected_text or detected_text in phrase
        )
        chosen = next(item for item in kwargs["candidates"] if item.label == target)
        return IntentAdjudicationResult(
            decision=IntentAdjudicationDecision.MATCH,
            selection_id=chosen.selection_id,
            candidate_set_hash=f"sha256:{kwargs['detected_text']}",
        )

    def adjudicate_many(self, **kwargs) -> IntentAdjudicationBatchResult:
        self.calls.append(kwargs)
        items = []
        for group in kwargs["groups"]:
            if any(phrase in group.detected_text for phrase in self.abstain_phrases):
                items.append(
                    IntentAdjudicationBatchItem(
                        detected_text=group.detected_text,
                        result=IntentAdjudicationResult(
                            decision=IntentAdjudicationDecision.NONE,
                            candidate_set_hash=f"sha256:{group.detected_text}",
                        ),
                    )
                )
                continue
            target = next(
                label
                for phrase, label in self.labels_by_phrase.items()
                if phrase in group.detected_text or group.detected_text in phrase
            )
            chosen = next(item for item in group.candidates if item.label == target)
            items.append(
                IntentAdjudicationBatchItem(
                    detected_text=group.detected_text,
                    result=IntentAdjudicationResult(
        ...
        return IntentAdjudicationBatchResult(
            items=tuple(items),
            candidate_set_hash="sha256:batch-intent-candidates",
        )
...
def _two_candidate_weak_sales_release(sales_release):
    release = _weak_sales_metric_release(sales_release)
    item_metric = MetricSpec(
        id="item_sales",
        name="商品成交金额",
        ...
        aliases=("商品销售额",),
        description="按订单明细统计的商品成交金额",
    )
    return release.model_copy(
        update={
            "metrics": (*release.metrics, item_metric),
            "datasets": tuple(
                item.model_copy(update={"metric_ids": (item_metric.id,)})
                if item.id == "item_scope"
                else item
...
def test_cross_type_weak_semantic_ai_match_completes_without_a_confirmation_card(
    sales_release,
) -> None:
    release = _routed_cross_type_same_name_release(sales_release)
    ...
    service, gateway, executor = _ambiguity_service(
        ...
        weak_mode=WeakMetricAdjudicationMode.AUTO,
    )
    ...
    assert adjudicator.calls[0]["intent_kind"] == "semantic_element"
    assert {(item.kind, item.label) for item in adjudicator.calls[0]["candidates"]} == {
        ("metric", "业务数量总计"),
        ("dimension", "业务数量总计"),
    }
    discovery = next(item for item in response.trace if item.stage.value == "CANDIDATE_DISCOVERY")
    assert discovery.detail["semantic_intent_adjudication"]["decision"] == "MATCH"
    assert discovery.detail["scope_resolution"]["ai_adjudicated_metric_ids"] == ["net_revenue"]
    assert len(response.semantic_decisions) == 1
    assert response.semantic_decisions[0].source == "ai"
    assert response.semantic_decisions[0].chosen.element_id == "net_revenue"


def test_ai_selected_weak_semantic_is_not_executed_when_final_s2sql_ignores_it(
    sales_release,
) -> None:
    release = _routed_cross_type_same_name_release(sales_release)
    adjudicator = _LabelChoosingIntentAdjudicator(
    ...
    final = next(item for item in reversed(response.trace) if item.stage.value == "FINAL_PARSING")
    assert final.detail["code"] == "SELECTED_SEMANTIC_NOT_USED"
    assert {item.kind for item in response.options} == {"metric", "dimension"}
    assert gateway.calls == 1
    assert executor.calls == 0


def test_cross_type_weak_semantic_ai_abstention_preserves_the_single_human_card(
    sales_release,
) -> None:
    release = _routed_cross_type_same_name_release(sales_release)
    adjudicator = _LabelChoosingIntentAdjudicator(decision=IntentAdjudicationDecision.AMBIGUOUS)
    service, gateway, executor = _ambiguity_service(
        release,
        intent_adjudicator=adjudicator,
        weak_mode=WeakMetricAdjudicationMode.AUTO,
    ...
    assert executor.calls == 0
    discovery = next(item for item in response.trace if item.stage.value == "CANDIDATE_DISCOVERY")
    assert discovery.detail["semantic_intent_adjudication"]["fallback"] == ("human_confirmation")
...
def test_typed_metric_continuation_with_a_cross_scope_same_bare_id_runs_selected_scope(
    sales_release,
) -> None:
    release = _cross_scope_same_bare_id_release(sales_release)
    index = SemanticIndexBuilder(_CountingEmbeddingGateway()).build(release)
    gateway = _FixedS2SqlGateway('SELECT SUM("业务数量总计") FROM "订单范围"')
    executor = _Executor()
    service = AnalyticsQueryService(
        releases=_ReleaseProvider(release, index),
        orchestrator=CandidateOrchestrator(
    ...
    assert response.state is QueryState.COMPLETED, response
    assert response.semantic_query.dataset_id == "orders_scope"
    assert response.semantic_query.metric_ids == ("shared",)
    assert gateway.calls == 1
    assert executor.calls == 1
...
def test_weak_metric_requires_business_confirmation_before_scope_selection(
    sales_release,
) -> None:
    """Reviewed global Mapper contract for incomplete business wording.

    The exact region value is reachable from two fact roots.  A keyword metric
    candidate must be shown as a business metric confirmation; the service must
    not expose either internal Scope or silently take the weak first match.
    """

    release = _weak_sales_metric_release(sales_release)
    llm = _FixedS2SqlGateway('SELECT SUM("净收入") FROM "销售经营" WHERE "区域" = \'华东\'')
    service, _embedding, executor = _service(
        release,
        llm_gateway=llm,
        query_embedding=False,
    ...
    assert first.state is QueryState.CLARIFICATION_REQUIRED, first.model_dump_json(indent=2)
    assert first.question == "你说的「销售额」是否指指标「净收入」？"
    ...
    assert option.kind == "metric"
    assert option.label == "净收入"
    assert "订单明细分析" not in first.model_dump_json()
    assert "销售经营" not in first.question
    assert llm.calls == 0
    assert executor.calls == 0
    ...
    assert discovery.detail["scope_resolution"]["owner_model_ids"] == ["orders"]
    assert discovery.detail["scope_resolution"]["exact_metric_ids"] == []
    assert discovery.detail["scope_resolution"]["confirmed_metric_ids"] == ["net_revenue"]
    assert discovery.detail["scope_resolution"]["exact_dimension_ids"] == ["region"]
    assert all(mode.startswith("sales_dataset:") for mode in discovery.detail["mapping_modes"])
    final = next(item for item in second.trace if item.stage.value == "FINAL_PARSING")
    confirmed_match = next(
        item
        for item in final.detail["final_mapping"]["matches"]
        if item["element_id"] == "net_revenue"
    )
    assert confirmed_match["method"] == "term_description"
    assert confirmed_match["detected_span_source"].startswith("term:")
    assert confirmed_match["detected_text"] == "净收入"
    assert llm.calls == 1
    assert executor.calls == 1


def test_ai_match_auto_routes_and_preserves_weak_metric_provenance(
    sales_release,
) -> None:
    release = _two_candidate_weak_sales_release(sales_release)
    adjudicator = _ScriptedWeakMetricAdjudicator(
        WeakMetricAdjudicationResult(
            decision=WeakMetricAdjudicationDecision.MATCH,
            metric_id="net_revenue",
            reason="订单净收入符合销售额口径",
            candidate_set_hash="sha256:test-candidates",
        )
    )
    llm = _FixedS2SqlGateway('SELECT SUM("净收入") FROM "销售经营" WHERE "区域" = \'华东\'')
    failures = _FailureStore()
    service, _embedding, executor = _service(
        release,
        llm_gateway=llm,
        query_embedding=False,
    ...
    assert response.semantic_query.metric_ids == ("net_revenue",)
    assert len(adjudicator.calls) == 1
    assert adjudicator.calls[0]["detected_text"] == "销售额"
    assert set(adjudicator.calls[0]["metric_ids"]) == {"net_revenue", "item_sales"}
    assert adjudicator.calls[0]["exact_context"] == ("区域 = 华东",)
    discovery = next(item for item in response.trace if item.stage.value == "CANDIDATE_DISCOVERY")
    assert discovery.detail["weak_metric_adjudication"]["decision"] == "MATCH"
    assert discovery.detail["scope_resolution"]["exact_metric_ids"] == []
    assert discovery.detail["scope_resolution"]["confirmed_metric_ids"] == []
    assert discovery.detail["scope_resolution"]["ai_adjudicated_metric_ids"] == ["net_revenue"]
    ...
    assert selected_match["method"] != "exact"
    assert llm.calls == 1
    assert executor.calls == 1
    assert all(record.code != "SEMANTIC_ELEMENT_SELECTED" for record, _kwargs in failures.saved)
...
@pytest.mark.parametrize(
    ...
    [
        (WeakMetricAdjudicationDecision.AMBIGUOUS, None),
        (WeakMetricAdjudicationDecision.NONE, None),
        (WeakMetricAdjudicationDecision.UNAVAILABLE, "MODEL_GATEWAY_FAILED"),
    ],
)
def test_ai_abstention_or_failure_falls_back_to_existing_human_confirmation(
    sales_release,
    decision,
    failure_code,
) -> None:
    ...
    adjudicator = _ScriptedWeakMetricAdjudicator(
        WeakMetricAdjudicationResult(
            decision=decision,
            reason="supersecret upstream detail" if failure_code else "无法唯一判断",
            candidate_set_hash="sha256:test-candidates",
    ...
    assert response.state is QueryState.CLARIFICATION_REQUIRED
    assert [item.label for item in response.options] == ["净收入"]
    assert response.options[0].kind == "metric"
    assert len(adjudicator.calls) == 1
    discovery = next(item for item in response.trace if item.stage.value == "CANDIDATE_DISCOVERY")
    assert discovery.detail["weak_metric_adjudication"]["decision"] == decision.value
    assert discovery.detail["weak_metric_adjudication"]["fallback"] == "human_confirmation"
    assert "supersecret upstream detail" not in response.model_dump_json()
    assert llm.calls == 0
    assert executor.calls == 0


def test_shadow_ai_match_still_uses_the_existing_human_confirmation(
    sales_release,
) -> None:
    adjudicator = _ScriptedWeakMetricAdjudicator(
        WeakMetricAdjudicationResult(
    ...
    service, _embedding, executor = _service(
        _weak_sales_metric_release(sales_release),
        query_embedding=False,
        weak_metric_adjudicator=adjudicator,
        adjudication_mode=WeakMetricAdjudicationMode.SHADOW,
    )
    ...
    assert response.state is QueryState.CLARIFICATION_REQUIRED
    discovery = next(item for item in response.trace if item.stage.value == "CANDIDATE_DISCOVERY")
    assert discovery.detail["weak_metric_adjudication"] == {
        "mode": "shadow",
        "decision": "MATCH",
        "candidate_set_hash": "sha256:test-candidates",
...
def test_explicit_metric_confirmation_is_reused_as_version_bound_memory(
    sales_release,
) -> None:
    release = _weak_sales_metric_release(sales_release)
    llm = _FixedS2SqlGateway('SELECT SUM("净收入") FROM "销售经营" WHERE "区域" = \'华东\'')
    ...
    question = "华东销售额是多少"
    semantic_now = datetime(2100, 1, 1, tzinfo=UTC)
    wall_before = datetime.now(UTC)

    first = service.query(
        QueryRequest(project_id="sales", question=question),
        now=semantic_now,
        actor_id="tenant-1",
    )
    assert first.state is QueryState.CLARIFICATION_REQUIRED
    second = service.query(
        QueryRequest(
            project_id="sales",
            question=question,
            selected_candidate_id=first.options[0].candidate_id,
            expected_release_id=first.release_id,
            expected_spec_hash=first.spec_hash,
            expected_index_snapshot_id=first.index_snapshot_id,
        ),
    ...
    assert memories.saved[0].semantic_element_id == "net_revenue"
    assert memories.saved[0].selection_kind == "metric"
    assert wall_before <= memories.saved[0].created_at <= datetime.now(UTC)
    assert memories.saved[0].expires_at.year < 2100
    ...
    assert third.semantic_query.metric_ids == ("net_revenue",)
    assert third.semantic_decisions[0].source == "memory"
    assert len(memories.saved) == 1
    assert llm.calls == 2
    assert executor.calls == 2
    discovery = next(item for item in third.trace if item.stage.value == "CANDIDATE_DISCOVERY")
    assert discovery.detail["confirmation_memory"]["status"] == "applied"


def test_confirmation_memory_never_crosses_actor_or_candidate_set(
    sales_release,
) -> None:
    release = _weak_sales_metric_release(sales_release)
    memories = _ConfirmationMemoryStore()
    ...
    assert selected.state is QueryState.COMPLETED

    other_actor = service.query(
        QueryRequest(project_id="sales", question=question),
        actor_id="tenant-2",
    )

    assert other_actor.state is QueryState.CLARIFICATION_REQUIRED
...
def test_forged_weak_metric_not_in_displayed_options_is_rejected(
    sales_release,
) -> None:
    release = _weak_sales_metric_release(sales_release)
    ...
    first = service.query(QueryRequest(project_id="sales", question="华东销售额是多少"))

    forged = service.query(
        QueryRequest(
            project_id="sales",
            question="华东销售额是多少",
            selected_candidate_id="element:metric:refund_amount",
            expected_release_id=first.release_id,
            expected_spec_hash=first.spec_hash,
            expected_index_snapshot_id=first.index_snapshot_id,
        )
...
def test_stale_metric_confirmation_is_rejected_before_mapping_or_execution(
    sales_release,
) -> None:
    release = _weak_sales_metric_release(sales_release)
    service, embedding, executor = _service(release, query_embedding=False)
    first = service.query(QueryRequest(project_id="sales", question="华东销售额是多少"))
...
def test_normal_metric_confirmation_response_redacts_internal_scope_details(
    sales_release,
) -> None:
    service, _embedding, _executor = _service(
        _weak_sales_metric_release(sales_release),
    ...
    response = service.query(QueryRequest(project_id="sales", question="华东销售额是多少"))

    payload = response.model_dump_json()
    assert response.state is QueryState.CLARIFICATION_REQUIRED
    assert "scope_resolution" not in payload
    assert "mapping_attempts" not in payload
    assert "item_scope" not in payload
    assert "sales_dataset" not in payload


def test_multiple_weak_metrics_are_all_business_options_not_a_first_wins_pick(
    sales_release,
) -> None:
    release = _weak_sales_metric_release(sales_release)
    item_metric = MetricSpec(
    ...
    assert {
        ...
        "element:metric:net_revenue",
        "element:metric:item_sales",
    }
    assert {item.kind for item in response.options} == {"metric"}
    assert {item.label for item in response.options} == {"净收入", "商品成交金额"}
    rendered = response.model_dump_json()
    assert "销售经营" not in response.question
    assert "订单明细分析" not in rendered
    assert executor.calls == 0


def test_independent_weak_metric_phrases_fail_closed_without_losing_one(
    sales_release,
) -> None:
    release = _weak_sales_metric_release(sales_release)
    item_metric = MetricSpec(
    ...
    service, _embedding, executor = _service(
        release,
        llm_gateway=_FixedS2SqlGateway('SELECT SUM("净收入"), SUM("退款金额") FROM "销售经营"'),
        query_embedding=False,
        weak_metric_adjudicator=adjudicator,
    )

    response = service.query(QueryRequest(project_id="sales", question="华东销售额和销量是多少"))
...
def test_multiple_weak_metric_phrases_auto_complete_only_when_all_share_one_root(
    sales_release,
) -> None:
    release = _weak_sales_metric_release(sales_release)
    adjudicator = _PhraseChoosingIntentAdjudicator(
        {"销售额": "净收入", "净收入": "净收入", "退款": "退款金额"}
    )
    llm = _FixedS2SqlGateway('SELECT SUM("净收入"), SUM("退款金额") FROM "销售经营"')
    service, _embedding, executor = _service(
        release,
        llm_gateway=llm,
        query_embedding=False,
    ...
    assert response.state is QueryState.COMPLETED, response.model_dump_json(indent=2)
    assert response.semantic_query.metric_ids == ("net_revenue", "refund_amount")
    assert {item.detected_text for item in response.semantic_decisions} == {
        "销售额",
        "退款",
    }
    assert len(adjudicator.calls) == 1
    assert len(adjudicator.calls[0]["groups"]) == 2
    assert executor.calls == 1


def test_multiple_ai_resolved_weak_phrases_across_roots_are_refused(
    sales_release,
) -> None:
    release = _weak_sales_metric_release(sales_release)
    item_metric = MetricSpec(
    ...
    response = service.query(
        QueryRequest(project_id="sales", question="销售额和销量分别是多少"),
        actor_id="tenant-1",
    )
...
def test_partial_multi_phrase_adjudication_returns_one_combination_card(
    sales_release,
) -> None:
    release = _weak_sales_metric_release(sales_release)
    memories = _ConfirmationMemoryStore()
    ...
    response = service.query(
        QueryRequest(project_id="sales", question="销售额和退款分别是多少"),
        actor_id="tenant-1",
    )
    ...
    assert all(len(item.semantic_selection_ids) == 2 for item in response.options)
    assert len(adjudicator.calls) == 1
    selected = next(
        item for item in response.options if item.label == "退款：退款金额；销售额：净收入"
    )
    continued = service.query(
        QueryRequest(
            ...
            expected_index_snapshot_id=response.index_snapshot_id,
        ),
        actor_id="tenant-1",
    )

    assert continued.state is QueryState.COMPLETED, continued.model_dump_json(indent=2)
    assert {item.detected_text for item in continued.semantic_decisions} == {
        "销售额",
        "退款",
    }
    assert {item.source.value for item in continued.semantic_decisions} == {"human"}
    assert executor.calls == 1

    replayed = service.query(
        QueryRequest(project_id="sales", question="销售额和退款分别是多少"),
        actor_id="tenant-1",
    )
    assert replayed.state is QueryState.COMPLETED, replayed.model_dump_json(indent=2)
    assert {item.source.value for item in replayed.semantic_decisions} == {"memory"}
    assert len(adjudicator.calls) == 1
    assert executor.calls == 2


def test_keyword_and_independent_embedding_metric_phrases_both_block_execution(
    sales_release,
) -> None:
    release = _weak_sales_metric_release(sales_release)
    item_metric = MetricSpec(
    ...
    embedding = _IndependentWeakPhraseEmbeddingGateway()
    index = SemanticIndexBuilder(embedding).build(release)
    executor = _Executor()
    service = AnalyticsQueryService(
        releases=_ReleaseProvider(release, index),
        orchestrator=CandidateOrchestrator(
            mapper=SemanticMapper(embedding_gateway=embedding),
        ),
        translator=SemanticTranslator(),
        executor=executor,
    )

    response = service.query(QueryRequest(project_id="sales", question="销售额和火星是多少"))
...
def test_weak_metric_candidate_overflow_fails_closed_without_truncation(
    sales_release,
) -> None:
    metrics = tuple(
        MetricSpec(
            id=f"sales_metric_{index}",
            name=f"销售口径{index}",
            model_id="orders",
            field_id="orders.net_amount",
            aggregation=Aggregation.SUM,
    ...
    routes = tuple(
        AnalysisTopicRouteSpec(dataset_id=dataset.id, root_model_id="orders")
    ...
    service, _embedding, executor = _service(release, query_embedding=False)

    response = service.query(QueryRequest(project_id="sales", question="销售口径是多少"))

    assert response.state is QueryState.CLARIFICATION_REQUIRED
    assert response.options == ()
    assert "候选业务指标过多" in response.question
    assert executor.calls == 0
...
def test_weak_metric_confirmation_precedes_a_conflicting_unique_dimension_root(
    sales_release,
) -> None:
    """A unique dimension root cannot hide a weak metric owned by another root."""

    release = _weak_sales_metric_release(sales_release).model_copy(
        update={
            "dimension_values": (
                *sales_release.dimension_values,
                DimensionValueSpec(
                    id="product_a",
    ...
    first = service.query(QueryRequest(project_id="sales", question="商品甲销售额是多少"))
    ...
    assert second.error.code == "DIMENSION_NOT_REACHABLE"
    assert llm.calls == 0
    assert executor.calls == 0


def test_ai_match_cannot_override_an_incompatible_exact_dimension(
    sales_release,
) -> None:
    release = _weak_sales_metric_release(sales_release).model_copy(
        update={
    ...
    discovery = next(item for item in response.trace if item.stage.value == "CANDIDATE_DISCOVERY")
    assert discovery.detail["weak_metric_adjudication"]["decision"] == "MATCH"
    assert discovery.detail["weak_metric_adjudication"]["governance_code"] == (
